...
public boolean equals(Object x){
	Matrix m;
	m = (Matrix)x;
	boolean equal = (m.getSize() == getSize());
	boolean entryeq = true;
	if(!equal){
		return false;
	}
	else{
...
public Matrix sub(Matrix M){
	...
	Matrix m = M.scalarMult(-1);//makes all entries with in sub to be -1(data)
	Matrix s = add(m);
...
private List adder(List P, List Q){
	P.moveFront();
	Q.moveFront();
	List l = new List();
	double a = 0;
	...
	while(P.index() >= 0 || Q.index() >= 0){
		if(P.index() >= 0 && Q.index() >= 0){//both P and Q have not fallen off the list
			...
			if(temp1.column < temp2.column){
				...
				l.append(new Entry(temp2.column, temp2.data));
				Q.moveNext();
			}
			else if(temp1.column == temp2.column){
				Entry e = new Entry(temp1.column , (temp1.data + temp2.data));
				if(e.data != 0){
					l.append(e);
				}
				P.moveNext();
		...
		else if(Q.index() >= 0){//if P has fallen off the list
			Entry temp2 = (Entry)Q.get();
			l.append(new Entry(temp2.column, temp2.data));
			Q.moveNext();
		}
	...
	for(int i = 1 ; i <= getSize() ; i++){
		...
		for(int j = 1 ; j <= getSize() ; j++){
			if(m.row[j].length() == 0){
				...
				T.changeEntry(i , j , dot(row[i], m.row[j]));
			}
		}
	}
	return T;
